(() => {
  ...
  // Microinterações e experiência de produto
  const backToTop = $('#backToTop');
  const updateBackToTop = () => backToTop?.classList.toggle('visible', (window.scrollY || 0) > 700);
  ...
  const promptOutput = $('#promptDemoOutput');
  let typingTimer = 0;
  function typePrompt(text) {
    ...
    typingTimer = window.setInterval(() => {
      promptOutput.textContent += text.charAt(index);
      index += 1;
      if (index >= text.length) window.clearInterval(typingTimer);
    }, 12);
  }

  $$('[data-prompt-tab]').forEach(tab => {
    tab.addEventListener('click', () => {
      $$('[data-prompt-tab]').forEach(item => {
        const active = item === tab;
        item.classList.toggle('active', active);
        item.setAttribute('aria-selected', String(active));
      });
      typePrompt(tab.dataset.example || '');
    });
  });
